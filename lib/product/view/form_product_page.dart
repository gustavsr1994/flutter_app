...
class _FormProductPageState extends State<FormProductPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(
            widget.id != 0 ? "Form Update Product" : "Form Insert New Product"),
      ),
      body: Consumer<FormProductController>(
        builder: (context, state, _) {
          return FutureBuilder(
              future: state.detailProduct(widget.id),
                      ...
                      children: [
                        ...
                        ElevatedButton(
                            onPressed: () => widget.id != 0
                                ? state.actionSubmitUpdate(context, widget.id)
                                : state.actionSubmit(context),
                            style: ElevatedButton.styleFrom(
                                backgroundColor: Colors.blue[900]),
                            child: Text(
                              "Submit",
